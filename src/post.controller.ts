import {
    Body, Get, Post as HttpPost, Route, Tags, Security, Request, Path, Query,
    Controller, Res, TsoaResponse, SuccessResponse
} from 'tsoa';
...
export class PostController extends Controller {
  ...
  @HttpPost('') // Endpoint untuk membuat post tetap sama (POST /api/v1/posts)
  public async createPost(
    ...
    if (!body.imageBase64 || !body.imageFileType) {
        return badRequestResponse(400, { message: "imageBase64 and imageFileType are required." });
    }

    // Validasi sederhana untuk imageFileType (bisa diperluas)
    if (!body.imageFileType.startsWith('image/')) {
        return badRequestResponse(400, { message: "Invalid imageFileType. Must be like 'image/jpeg' or 'image/png'."});
    }
    ...
    const base64PrefixMatch = body.imageBase64.match(/^data:(image\/\w+);base64,/);
    ...
    try {
        ...
        const s3UploadResult = await uploadBase64ToS3(base64Data, body.imageFileType);

        // 2. Simpan informasi post ke database dengan URL S3
        const [newPost] = await db.insert(posts) // 'posts' adalah objek skema dari db.ts
          .values({
        ...
        const userProfile = await db.query.users.findFirst({ // 'users' adalah objek skema
        ...
        if (error.message.includes("S3") || error.message.includes("configured")) {
             return serverErrorResponse(500, { message: `Failed to process image: ${error.message}` });
        }
        return serverErrorResponse(500, { message: error.message || "Failed to create post." });
    }
  }
  ...
  public async searchPosts(
    ...
    if (!query || query.trim() === '') {
      return badRequestResponse(400, { message: 'Search query cannot be empty' });
    }
    ...
    .from(posts)
    .innerJoin(users, eq(posts.userId, users.id))
    .where(sql`"${customSchema.schemaName}".ins_posts.search_vector @@ plainto_tsquery('english', ${searchTerm})`)
    .orderBy(sql`ts_rank_cd("${customSchema.schemaName}".ins_posts.search_vector, plainto_tsquery('english', ${searchTerm})) DESC`)
    .limit(limit)
    .offset(offset);
    return results;
  }
