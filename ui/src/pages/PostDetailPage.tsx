...
const PostDetailPage: React.FC = () => {
  ...
  const handleCommentSubmit = async (e: FormEvent) => {
    ...
    setCommentError(null);
    try {
      const createdComment = await apiClient.createComment(post.id, {
        text: newComment,
      });
  ...
  if (postError && !post)
    return (
      <div className="container mx-auto p-4 text-center text-red-500 bg-red-50 border border-red-200 rounded-md">
        {postError}
      </div>
    );
  if (!post)
  ...
  return (
    <div className="container mx-auto p-2 sm:p-4 max-w-4xl">
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden md:flex">
        ...
        <div className="md:w-1/2 lg:w-2/5 p-4 flex flex-col h-full max-h-[80vh]">
          {" "}
          ...
          {/* Caption & Date */}
          <div className="mb-3 pr-2 custom-scrollbar overflow-y-auto max-h-28">
            {" "}
            {/* Scrollable caption */}
            {post.caption && (
              <p className="text-sm text-gray-700 leading-relaxed">
                <RouterLink
                  to={`/profile/${post.userId}`}
                  className="font-semibold hover:underline mr-1 text-gray-800"
                >
                  {post.username}
                </RouterLink>
                {post.caption}
